...
---@type table<string, Continue.ExtensionHandler>
local handlers = {}

---@param handler Continue.ExtensionHandler
...
function M.init(opts)
  for key, value in pairs(opts.extensions) do
    if value then
      M.register(value())
    end
  end
...
function M.load(session_opts)
  ...
  if not success then
    logger.error("Session data generated by extensions got corrupted. Can't load it")
    return
  end

  for key, value in pairs(data) do
    logger.error("loading data for %s", key)
